...
rsync -av /home/user/git/test.xmpp.is/etc/prosody/ /etc/prosody/

# Hiawatha config
echo
echo "Syncing Hiawatha configs.."
rsync -av /home/user/git/test.xmpp.is/etc/hiawatha/ /etc/hiawatha/

# Tor config
echo
echo "Syncing Tor configs..."
rsync -av /home/user/git/test.xmpp.is/etc/tor/ /etc/tor/

# Webroot
echo
echo "Syncing /var/www...."
rsync -av /home/user/git/test.xmpp.is/var/www/ /var/www/

# Mercurial
echo
echo "Syncing /etc/mercurial....."
rsync -av /home/user/git/test.xmpp.is/etc/mercurial/ /etc/mercurial/
...
crontab /home/user/git/test.xmpp.is/var/spool/cron/crontabs/root
cp /home/user/git/test.xmpp.is/etc/cron.d/certbot /etc/cron.d/certbot
